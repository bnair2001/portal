...
function toObject(arr,len) {
  var rv = {};
  for (var i = 0; i < arr.length; ++i)
    rv[i+len] = arr[i].name;
  return rv;
}
export const updateArchive = (archive, files) => {
  return async (dispatch, getState, { getFirestore }) => {
    const firestore = getFirestore();
    
    let size= Object.keys(archive.files).length;
     let obj=toObject(files,size)
     archive.files = {...archive.files, ...obj};
